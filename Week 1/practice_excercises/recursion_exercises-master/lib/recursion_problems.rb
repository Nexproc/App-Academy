...
def includes?(array, target)
	...
	array.empty? ? false : (array[0] == target || includes?(array.drop(1), target))
end
...
def num_occur(array, target)
	#return 0 if array.empty?
	array.empty? ? 0 : (num_occur(array.drop(1), target) + (array.first == target ? 1 : 0))
end
...
def reverse(string)
	...
	string.length <= 1 ? string : reverse(string[1..string.length]) + string[0]
end
