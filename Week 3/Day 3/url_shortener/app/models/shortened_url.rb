class ShortenedUrl < ActiveRecord::Base
  ...
  def self.prune(n)
    for_removal = self.where("created_at < ? ", n.minutes.ago)
    for_removal = for_removal.where(submitter: User.where(premium: false))
    for_removal.destroy_all
  end

  def self.user_defined_url!(user, long_url)
