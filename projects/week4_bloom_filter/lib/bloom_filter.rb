...
class BloomFilter
  ...
  def insert(key)
    ...
    key = key.hash
    flip = false
    self.hash_num.times do |i|
      key = hash(key) + i
      # Then return true or false and/or increment count, depending on whether
      # any bits were flipped.
      unless self.bit_array[hashed_index(key)]
        flip = true
        @bits_flipped += 1
        self.bit_array[hashed_index(key)] = true
      end
    end
    #
  ...
  def include?(key)
    ...
    # :bit_array.any? {|val| hash(key) == val}
  end

  def clear
    # Clean out your Bloom Filter, you filthy child.
    :bit_array.map! {|val| val = 0}
  end
  ...
  def hashed_index(key, i)
    hash(somethingSomething ^ key.hash) % self.num_bits
  ...
  class Scalable
    ...
    def count
      # potentially inaccurate count due to double positives
